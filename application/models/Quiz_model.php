...
Class Quiz_model extends CI_Model
{
 ...
 function insert_quiz(){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->insert('examenes_quiz',$userdata);
 ...
 function update_quiz($quid){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->update('examenes_quiz',$userdata);
	  
	  $this->db->where('quid',$quid);
	  $query=$this->db->get('examenes_quiz',$userdata);
	 $quiz=$query->row_array();
	 if($quiz['question_selection']=='1'){
		 
	  $this->db->where('quid',$quid);
	  $this->db->delete('examenes_qcl');
	 ...
	 foreach($_POST['cid'] as $ck => $val){
		 ...
		 $this->db->insert('examenes_qcl',$userdata);
		 }
		 }
	 }
		 $userdata=array(
...
/*
	 if($cid!='0'){
		 $this->db->where('examenes_qbank.cid',$cid);
	 }
	 if($lid!='0'){
		 $this->db->where('examenes_qbank.lid',$lid);
	 }
*/
	  
	 $query=$this->db->query("select * from examenes_qbank join examenes_category on examenes_category.cid=examenes_qbank.cid join examenes_level on examenes_level.lid=examenes_qbank.lid 
	 where examenes_qbank.qid in ($qids) order by FIELD(examenes_qbank.qid,$qids) 
	 ");
	 return $query->result_array();
 ...
 function get_options($qids){
	 ...
	 $query=$this->db->query("select * from examenes_options where qid in ($qids) order by FIELD(examenes_options.qid,$qids)");
	 return $query->result_array();
...
function get_qcl($quid){
	 ...
	 $this->db->where('quid',$quid);
	 $query=$this->db->get('examenes_qcl');
	 return $query->result_array();
 ...
 function remove_quiz($quid){
	 ...
	 $this->db->where('quid',$quid);
	 if($this->db->delete('examenes_quiz')){
...
$noq=$quiz['noq'];	
	...
	$wqids=implode(',',$qids);
	$noq=array();
	$query=$this->db->query("select * from examenes_qbank join examenes_category on examenes_category.cid=examenes_qbank.cid where qid in ($wqids) ORDER BY FIELD(qid,$wqids)  ");	
	...
	foreach($qcl as $k => $val){
		...
		$i=0;
	$query=$this->db->query("select * from examenes_qbank join examenes_category on examenes_category.cid=examenes_qbank.cid where examenes_qbank.cid='$cid' and lid='$lid' ORDER BY RAND() limit $noq ");	
	$questions=$query->result_array();
	foreach($questions as $qk => $question){
		$qids[]=$question['qid'];
		if(!in_array($question['category_name'],$categories)){
	...
	$zeros=array();
	 ...
	 $this->db->insert('examenes_result',$userdata);
 ...
 function open_result($quid,$uid){
	 $result_open=$this->lang->line('open');
		$query=$this->db->query("select * from examenes_result  where examenes_result.result_status='$result_open' "); 
	if($query->num_rows() >= '1'){
		$result=$query->row_array();
return $result['rid'];		
	}else{
 ...
 function quiz_result($rid){
	...
	$query=$this->db->query("select * from examenes_result join examenes_quiz on examenes_result.quid=examenes_quiz.quid where examenes_result.rid='$rid' "); 
	return $query->row_array(); 
...
function saved_answers($rid){
	...
	$query=$this->db->query("select * from examenes_answers  where examenes_answers.rid='$rid' "); 
	return $query->result_array(); 
 ...
 function assign_score($rid,$qno,$score){
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 $this->db->update('examenes_result',$userdata);
	 ...
	 $qp=$r_qids[$qno];
		...
		if($$qp_score=='1'){
		  ...
		  $query_qp="update examenes_qbank set  $crin  where qid='$qp'  ";
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 foreach($qids_perf as $qp => $qpval){
		 ...
		 if($qpval=='0'){
		  ...
		  $query_qp="update examenes_qbank set no_time_served=(no_time_served +1)  $crin  where qid='$qp'  ";
	 $this->db->query($query_qp);
...
if($this->config->item('allow_result_email')){
	...
	$query = $this -> db -> query("select examenes_result.*,examenes_users.*,examenes_quiz.* from examenes_result, examenes_users, examenes_quiz where examenes_users.uid=examenes_result.uid and examenes_quiz.quid=examenes_result.quid and examenes_result.rid='$rid'");
	$qrr=$query->row_array();
	...
	$this->db->delete('examenes_answers');
	 ...
	 foreach($_POST['answer'] as $ak => $answer){
		 ...
		 if($_POST['question_type'][$ak] == '1' || $_POST['question_type'][$ak] == '2'){
			 ...
			 $qid=$qids[$ak];
			 $query=$this->db->query(" select * from examenes_options where qid='$qid' ");
			 ...
			 $marks=0;
				foreach($answer as $sk => $ansval){
					...
					$this->db->insert('examenes_answers',$userdata);
				$attempted=1;	
				}
				if($attempted==1){
					if($marks >= '0.99' ){
