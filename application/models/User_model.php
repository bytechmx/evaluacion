...
Class User_model extends CI_Model
{
 function login($username, $password)
 {
   ...
   $this -> db -> where('examenes_users.password', MD5($password));
   }
    $this -> db -> where('examenes_users.email', $username);
    $this -> db -> where('examenes_users.verify_code', '0');
    $this -> db -> join('examenes_group', 'examenes_users.gid=examenes_group.gid');
  $this->db->limit(1);
    $query = $this -> db -> get('examenes_users');
 ...
 function user_list($limit){
	 if($this->input->post('search')){
		...
		$this->db->order_by('examenes_users.uid','desc');
		 $this -> db -> join('examenes_group', 'examenes_users.gid=examenes_group.gid');
		 $query=$this->db->get('examenes_users');
 ...
 function group_list(){
	...
	$query=$this->db->get('examenes_group');
		return $query->result_array();
 ...
 function verify_code($vcode){
	...
	$query=$this->db->get('examenes_users');
		if($query->num_rows()=='1'){
			...
			$this->db->update('examenes_users',$userdata);
			return true;
		}else{
 ...
 function insert_user(){
		...
		if($this->db->insert('examenes_users',$userdata)){
...
$queryr=$this->db->get('examenes_users');
...
$new_password=rand('1111','9999');
 ...
 if($this->config->item('protocol')=="smtp"){
			...
			$this->db->where('email', $toemail);
 			$this->db->update('examenes_users',$user_detail);
			return true;
			}
 ...
 function update_user($uid){
	 $logged_in=$this->session->userdata('logged_in');
		...
		if($this->input->post('password')!=""){
		 ...
		 $this->db->where('uid',$uid);
		if($this->db->update('examenes_users',$userdata)){
 ...
 function update_group($gid){
		...
		if($this->db->update('examenes_group',$userdata)){
 ...
 function remove_user($uid){
	 ...
	 $this->db->where('uid',$uid);
	 if($this->db->delete('examenes_users')){
		 return true;
	 }else{
 ...
 function remove_group($gid){
	 ...
	 $this->db->where('gid',$gid);
	 if($this->db->delete('examenes_group')){
		 return true;
	 }else{
 ...
 function get_user($uid){
	...
	$this->db->where('examenes_users.uid',$uid);
	   $this -> db -> join('examenes_group', 'examenes_users.gid=examenes_group.gid');
$query=$this->db->get('examenes_users');
	 return $query->row_array();
 ...
 function insert_group(){
		...
		if($this->db->insert('examenes_group',$userdata)){
