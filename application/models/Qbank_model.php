...
Class Qbank_model extends CI_Model
{
  ...
  function question_list($limit,$cid='0',$lid='0'){
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('examenes_qbank.qid',$search);
		 $this->db->or_like('examenes_qbank.question',$search);
		 $this->db->or_like('examenes_qbank.description',$search);
	 ...
	 if($lid!='0'){
		 ...
		 $this->db->join('examenes_category','examenes_category.cid=examenes_qbank.cid');
	 $this->db->join('examenes_level','examenes_level.lid=examenes_qbank.lid');
	 $this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('examenes_qbank.qid','desc');
		$query=$this->db->get('examenes_qbank');
 ...
 function get_option($qid){
	 $this->db->where('qid',$qid);
	 $query=$this->db->get('examenes_options');
	 return $query->result_array();
 ...
 function remove_question($qid){
	 ...
	 $this->db->where('qid',$qid);
	 if($this->db->delete('examenes_qbank')){
		  $this->db->where('qid',$qid);
			$this->db->delete('examenes_options');
		 return true;
	 }else{
 ...
 function insert_question_1(){
	 ...
	 'question_type'=>('Opción mútiple con respuesta unica'),
	 'cid'=>$this->input->post('cid'),
	 'lid'=>$this->input->post('lid')	 
	 );
	 $this->db->insert('examenes_qbank',$userdata);
	...
	$userdata=array(
	 ...
	 $this->db->insert('examenes_options',$userdata);	 
 ...
 function insert_question_2(){
	 ...
	 'question'=>$this->input->post('question'),
	 'description'=>$this->input->post('description'),
	 'question_type'=>('Opción múltiple respuesta multiple'),
	 'cid'=>$this->input->post('cid'),
	 'lid'=>$this->input->post('lid')	 
	 );
	 $this->db->insert('examenes_qbank',$userdata);
 ...
 function insert_question_5(){
  ...
  function update_question_1($qid){
	 ...
	 $this->db->where('qid',$qid);
	 $this->db->update('examenes_qbank',$userdata);
	 $this->db->where('qid',$qid);
	$this->db->delete('examenes_options');
	 foreach($this->input->post('option') as $key => $val){
 ...
 function category_list(){
	 ...
	 $query=$this->db->get('examenes_category');
	 return $query->result_array();
 ...
 function update_category($cid){
		...
		if($this->db->update('examenes_category',$userdata)){
 ...
 function remove_category($cid){
	 ...
	 $this->db->where('cid',$cid);
	 if($this->db->delete('examenes_category')){
		 return true;
	 }else{
 ...
 function insert_category(){
		...
		if($this->db->insert('examenes_category',$userdata)){
...
// level function start
 function level_list(){
	  $query=$this->db->get('examenes_level');
	 return $query->result_array();
 ...
 function update_level($lid){
		...
		if($this->db->update('examenes_level',$userdata)){
 ...
 function remove_level($lid){
	 ...
	 $this->db->where('lid',$lid);
	 if($this->db->delete('examenes_level')){
		 return true;
	 }else{
 ...
 function insert_level(){
		...
		if($this->db->insert('examenes_level',$userdata)){
...
$question_type=('Respuesta larga');	
	...
	if($this->db->insert('examenes_qbank',$insert_data)){
		$qid=$this->db->insert_id();
		$optionkeycounter = 4;
		if($ques_type=="0" || $ques_type==""){
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->insert("examenes_options",$insert_options);
				$optionkeycounter++;
				}
