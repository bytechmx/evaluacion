...
class Quiz extends CI_Controller {
	...
	function remove_qid($quid,$qid){
		...
		if($this->quiz_model->remove_qid($quid,$qid)){
                        $this->session->set_flashdata('message', "<div class='alert alert-success'>".('Se ha eliminado')." </div>");
		}
		...
		public function add_question($quid,$limit='0',$cid='0',$lid='0')		
	{
		...
		$logged_in=$this->session->userdata('logged_in');
		 ...
		 $data['quiz']=$this->quiz_model->get_quiz($quid);
		$data['title']=('Agregar pregunta a la evaluación').': '.$data['quiz']['quiz_name'];
		if($data['quiz']['question_selection']=='0'){
	...
	public function remove_quiz($quid){
			...
			if($this->quiz_model->remove_quiz($quid)){
                        $this->session->set_flashdata('message', "<div class='alert alert-success'>".('Se ha eliminado')." </div>");
						    ...
						    $this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Error al eliminar')." </div>");
	...
	public function quiz_detail($quid){
		...
		$logged_in=$this->session->userdata('logged_in');
		$gid=$logged_in['gid'];
		$data['title']=('Inteento').' '.('Evaluación');
	...
	public function validate_quiz($quid){
		...
		$this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Evaluación no asignada a tu grupo')." </div>");
		redirect('quiz/quiz_detail/'.$quid);
		 }
		// validate start end date/time
		if($data['quiz']['start_date'] > time()){
		$this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Evaluación no disponible')." </div>");
		redirect('quiz/quiz_detail/'.$quid);
		 }
		// validate start end date/time
		if($data['quiz']['end_date'] < time()){
		$this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Evaluación finalizada')." </div>");
		redirect('quiz/quiz_detail/'.$quid);
		 }
		...
		$this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Ip declinada')." </div>");
		redirect('quiz/quiz_detail/'.$quid);
		 }
		}
		...
		$this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Intengos maximos alcanzados')." </div>");
		redirect('quiz/quiz_detail/'.$quid);
		 }
	...
	function attempt($rid){
		...
		$this->quiz_model->submit_result($rid);
		$this->session->unset_userdata('rid');
		$this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Tiempo terminado')." </div>");
		redirect('quiz/quiz_detail/'.$data['quiz']['quid']);
		 }
...
if(isset($_FILES['webcam'])){
 ...
 function submit_quiz(){
				...
				if($this->quiz_model->submit_result()){
                        $this->session->set_flashdata('message', "<div class='alert alert-success'>".('Evaluación enviada')." </div>");
					}else{
						    $this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Error al enviar')." </div>");
