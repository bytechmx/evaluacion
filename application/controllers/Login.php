...
class Login extends CI_Controller {
	...
	{
		$data['title']=('Registrar nueva cuenta');
		// fetching group list
	...
	public function verifylogin(){
		...
		if($this->user_model->login($username,$password)){
			...
			// invalid login
			$this->session->set_flashdata('message', ('Email o contraseña invalidos'));
			redirect('login');
		}
	...
	function forgot(){
			...
			$user_email=$this->input->post('email');
			 if($this->user_model->reset_password($user_email)){
				$this->session->set_flashdata('message', "<div class='alert alert-success'>".('Password actualizado')." </div>");
						
			}else{
				$this->session->set_flashdata('message', "<div class='alert alert-danger'>".('Email no existe')." </div>");
