...
class Qbank extends CI_Controller {
	...
	public function index($limit='0',$cid='0',$lid='0')
	{
		...
		$data['title']=('Colección de preguntas');
		// fetching user list
	...
	public function pre_new_question()
	{
		...
		redirect('qbank/new_question_5/'.$nop);
		 ...
		 $data['title']=('Agregar nueva').' '.('pregunta');
		 $this->load->view('header',$data);
	...
	public function new_question_1($nop='4')
	{
			...
			if($this->input->post('question')){
                ...
                $this->session->set_flashdata('message', "<div class='alert alert-success'>Subido correctamente</div>");
				}else{
				 $this->session->set_flashdata('message', "<div class='alert alert-danger'>Error al subir datos</div>");
				}
		...
		public function edit_question_1($qid)
	{
			...
			if($this->input->post('question')){
                ...
                $this->session->set_flashdata('message', "<div class='alert alert-success'>Se actualizo correctamente</div>");
				}else{
				 $this->session->set_flashdata('message', "<div class='alert alert-danger'>Error al actualizar</div>");
				}
			...
			public function edit_question_5($qid)
	{
			...
			if($this->input->post('question')){
                ...
                $this->session->set_flashdata('message', "<div class='alert alert-success'>Se actualizo correctamente</div>");
				 ...
				 $this->session->set_flashdata('message', "<div class='alert alert-danger'>Error al subir actualización</div>");
				}
			...
			public function update_category($cid)
	{
			...
			if($logged_in['su']!='1'){
                ...
                echo "<div class='alert alert-success'>Se actualizo correctamente</div>";
				}else{
				 echo "<div class='alert alert-danger'>Error al subir actualización</div>";
...
$this->qbank_model->import_question($allxlsdata);   
  ...
  $this->session->set_flashdata('message', "<div class='alert alert-success'>Información importada</div>");
  redirect('qbank');
	}
